...
data Store s = Store s deriving (Show,Eq,Data,Typeable)
...
toExerciseDatum (StoreDatum (Store s)) = ExerciseStoreDatum (encodeJSON s)
...
toDatum (ExerciseStoreDatum s) = (StoreDatum . Store) <$> decode s
